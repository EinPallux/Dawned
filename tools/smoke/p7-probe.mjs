#!/usr/bin/env node
/**
 * P7-B live probe (temporary — superseded by browser-p7 at P7-E): drives the
 * progression wire end to end against a running dev server.
 *   1. ProgressSync follows the Welcome with the fresh-character sheet.
 *   2. /ops/setlevel → LevelUp + ProgressSync with banked points + refill.
 *   3. AllocateStats spends the bank; the sync reflects it.
 *   4. AllocateSkill without published nodes refuses cleanly (still syncs).
 *   5. A relog sees the PERSISTED level + allocation (write-through proof).
 *
...
const main = async () => {
  ...
  // 4. Unknown node refuses but still answers with a sync (self-heal path).
  client.socket.send(encodeAllocateSkill({ nodeId: 'node_warrior_bulwark_toughened' }));
  const afterSkill = await client.waitSyncs(client.progressSyncs.length + 1, 'skill refusal');
  if (Object.keys(afterSkill.nodes).length !== 0 || afterSkill.unspentSkillPoints !== 9) {
    fail('skill allocation should refuse with no published nodes');
  }
  ok('AllocateSkill without published nodes refused, sync self-heals');
  ...
  ok(`relog sees persisted level ${persisted.level} + allocation — write-through holds`);
  again.close();
