...
| Token expired (git pull fails after ~1 year) | make a new token, then: `sudo -u dawned git -C /opt/dawned/game remote set-url origin "https://EinPallux:NEW_TOKEN@github.com/EinPallux/Dawned.git"` (and the same for `/opt/dawned/admin` with the Dawned-Admin URL — UPDATE.sh refreshes the admin build token from that remote automatically) |
| Admin build fails fetching `@dawned/shared` (403/404 from codeload.github.com) | the panel's pinned shared-package download needs the PAT: check `/opt/dawned/admin`'s remote embeds a valid token (row above), then rerun `sudo bash /opt/dawned/game/deploy/UPDATE.sh admin` |
