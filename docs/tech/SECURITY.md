...
The client is a _renderer with opinions_. Server-side truth for everything that matters:
...
| Ability cheats (cooldown/cost/range) | Full re-validation server-side per COMBAT.md §4 pipeline; client numbers are never read — only _requests_ with aim                                                                                                                                         |
