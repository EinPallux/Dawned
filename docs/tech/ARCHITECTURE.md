...
| `/ops/camps`          | P12   | GET what the bestiary became: camps seeded, enemies wanted vs alive, per-zone population, unresolved enemy refs and — the line that matters — camps that produced NOTHING, which is how 124 camps published into open water would look identical to 124 on land |
...
| `/ops/forget`         | P11   | un-find a player's POIs (and optionally zones, shrines, or the chests and props they have used) — discovery is first-entry-only by design, so without this the loop can be measured exactly once per character, ever                                            |
