...
| **Inventory (`I`)**         | 48-slot grid + paper-doll (3D preview torso-cam) + gold + weight-free (no encumbrance) + sort/search + junk-sell hint. Drag ghosts are angular cut cards. _As built (P8): the paper-doll is icon slots rather than a 3D torso-cam — the 3D preview waits for a phase that needs visible armour; everything else shipped, plus tooltips that compare against the equipped piece._                    |
