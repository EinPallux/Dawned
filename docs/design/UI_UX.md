...
- **Micro menu** (owner-requested, P4 round 6): slim faceted icon strip anchored bottom-right
  ...
  every screen the keyboard already opens; **shipped at P7 with the C/K glyphs** (badges =
  banked stat/skill points), grows icons as later phases add theirs.
...
| **Inventory (`I`)**         | 48-slot grid + gold + weight-free (no encumbrance) + sort/search + junk-sell hint. Drag ghosts are angular cut cards. _As built (P8, revised 2026-08-04 on owner feedback): the pack is the bag ALONE — worn gear lives on the character sheet (`C`), where it sits next to the character it is on. Tooltips compare against the equipped piece from either panel._                                                                                                    |
| **Character (`C`)**         | Name + level header, worn gear in two slot columns flanking the live 3D rig (holding the weapons you equipped), then the stat block: attributes with +/− staging & confirm on the left, derived stats with hover formulas (transparency!) on the right, gear contributions marked `(+n)`. _As built (2026-08-04, owner reference): this screen owns EQUIPMENT — the pack is only the bag. Resistances, playtime and titles arrive with the systems that produce them._ |
| **Skills (`K`)**            | Left: 8 ability tiles with rank/unlock states (as-built P7: hotbar order is content-bound per slot — the panel decides it — so tiles inform rather than drag-to-rearrange); right: 3-branch tree — vertical faceted lattice per branch, nodes as cut hexes, connectors light up on invest, capstone at lattice crown; respec button (Mirror of Dawn deep-link note).                                                                                                   |
