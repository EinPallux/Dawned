...
- **Structure:** every panel = title bar strip (angular tab) + content on a subtle 2-tone split;
  ...
  Kenney Fantasy UI Borders reserved for *accents* (tooltips seams, dialogue frame) only.
- **Palette:**
...
| **Character Create** | 3-step flow in one screen: (1) class carousel — 4 large cards, hover = ability sizzle list + playstyle line, selected class poses in diorama with signature anim loop; (2) look — body (M/F), outfit (Ranger/Peasant set recolors), hairstyle (7) + color (8 swatches), eyebrow/beard toggles; (3) name + claim. Random-look dice button. All changes live on the 3D model. |
