...
- Each step: damage coefficient, small resource *generation* (Warrior Rage +4, Rogue Energy is
...
Out-of-combat (5 s no give/take damage): +8%/s HP, +10%/s resource (Rage instead *decays* 2/s always
...
- Grammar: *circle under you* = move anywhere; *cone from boss* = strafe behind; *expanding ring* =
  gap-close or dodge-through; *full-arena + safe wedge* = find the wedge (bosses only).
