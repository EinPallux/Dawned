...
export class ServerPlayer {
  ...
  lastProcessedSeq = 0;
  /** Ticks where the client's input arrived too late (network jitter indicator). */
  starvedTicks = 0;
   ...
   * intent repeats when nothing arrived, minus one-shot buttons.
   */
  takeInputsForTick(): MovementIntent[] {
    ...
    if (available === 0) {
      ...
      const repeat: MovementIntent = {
        ...this.lastIntent,
        buttons: this.lastIntent.buttons & ~ONE_SHOT_BUTTONS,
      };
